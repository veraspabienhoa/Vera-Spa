...
export default function SpaManagementPage({ user, mode, initialTab = 'services', embedded = false }) {
  ...
  return <div className="feature-page spa-management">
    ...
    <section className="panel spa-content" id="spa-settings-content" role={customersPage || embedded ? undefined : 'tabpanel'} aria-labelledby={customersPage || embedded ? undefined : `spa-${tab}-tab`}>
      ...
      {customersPage ? <div className="responsive-data-table"><table><thead><tr><th>Khách hàng</th><th>Điện thoại</th><th>Combo còn lại</th><th>Thao tác</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td><strong>{item.name || 'Chưa có tên'}</strong></td><td>{item.phone || '—'}</td><td>{(item.combo_purchases || []).reduce((sum, purchase) => sum + Number(purchase.remaining || 0), 0)} vé</td><td><div className="spa-actions spa-customer-actions"><button className="secondary-button" disabled={busy || !canEditCustomer} onClick={() => { setError(''); setCustomerContext({ customer: item, mode: 'edit', revision: data.revision }) }}>Sửa</button>{canCustomer('customers_delete') && <button className="secondary-button danger-button" disabled={busy} onClick={() => { setError(''); setCustomerContext({ customer: item, mode: 'delete', revision: data.revision }) }}>Xóa</button>}
      {(item.combo_purchases || []).map(purchase => <span key={purchase.id}>{purchase.combo_name} · {purchase.remaining} vé {user?.role === 'admin' && canCustomer('customer_combo_edit') && <button className="text-button" disabled={busy} onClick={() => { setError(''); setCustomerContext({ customer: item, purchase, mode: 'edit', revision: data.revision }) }}>Sửa combo</button>}{canCustomer('customer_combo_delete') && <button className="text-button" disabled={busy} onClick={() => { setError(''); setCustomerContext({ customer: item, purchase, mode: 'delete', revision: data.revision }) }}>Xóa combo</button>}</span>)}
      <button className="secondary-button spa-history-button" disabled={busy} onClick={() => history(item)}><History size={14}/> Lịch sử</button></div></td></tr>)}</tbody></table></div>
        : tab === 'services' ? <div className="responsive-data-table"><table><thead><tr><th aria-label="Sắp xếp"></th><th>Dịch vụ / nhóm</th><th>Loại dịch vụ</th><th>Thời lượng / thành phần</th><th>Giá</th><th>Số lượt</th><th>Admin duyệt bán</th><th>Thao tác</th></tr></thead><tbody>{filtered.map((item) => <tr key={`${item.catalog_kind}:${item.id}`} {...dragProps(item)}>
