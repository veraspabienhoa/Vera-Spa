...
function Editor({ title, onClose, busy, children }) {
  ...
  return <dialog ref={dialog} className="spa-editor" aria-label={title} onCancel={(event) => { event.preventDefault(); if (!busy) onClose() }}>
    <div data-ui-key="u-fe840576035a" className="spa-editor-heading"><h2>{title}</h2><button data-ui-key="u-6c82f382eb53" className="icon-button" type="button" aria-label="Đóng" disabled={busy} onClick={onClose}><X size={20}/></button></div>
    {children}
  </dialog>
}
...
export default function SpaManagementPage({ user, mode, initialTab = 'services', embedded = false }) {
  ...
  const toggleComboApproval = (item, checked) => {
    const next = newCatalogForm('combo', item)
  ...
  return <div className="feature-page spa-management">
    ...
    <section data-ui-key="u-dc002737a58a" className="panel spa-content" id="spa-settings-content" role={customersPage || embedded ? undefined : 'tabpanel'} aria-labelledby={customersPage || embedded ? undefined : `spa-${tab}-tab`}>
      ...
      <UiToolbar data-ui-key="u-0c85641eb827" className="spa-toolbar"><ClearableSearchInput type="search" aria-label="Tìm kiếm" placeholder={customersPage ? 'Tìm tên hoặc số điện thoại…' : 'Tìm theo tên…'} value={search} onChange={(event) => setSearch(event.target.value)}/><span>{filtered.length} / {rows.length}</span><UiToolbar data-ui-key="u-931eb3f43d58" className="spa-actions">{customersPage && data?.can_export && <button data-ui-key="u-10bcd2114caf" data-ui-label-default="Xuất Excel" className="secondary-button" disabled={busy} onClick={exportCustomers}><Download size={16}/><UiCustomText uiKey="u-10bcd2114caf"> Xuất Excel</UiCustomText></button>}<button data-ui-key="u-3666d91707cb" className="primary-button" disabled={busy || !data || (customersPage && !canCreateCustomer)} onClick={() => openEditor(addKind)}><Plus size={16}/> Thêm {customersPage ? 'khách hàng' : tab === 'services' ? 'dịch vụ' : 'khu vực'}</button></UiToolbar></UiToolbar>
      ...
      {customersPage ? <div className="responsive-data-table spa-fit-table"><table data-ui-key="u-3c4516c2163d" className="spa-customers-table"><thead><tr><th data-ui-key="u-d093516213d1" data-ui-label-default="Khách hàng"><UiCustomText uiKey="u-d093516213d1">Khách hàng</UiCustomText></th><th data-ui-key="u-de9bff6a0980" data-ui-label-default="Điện thoại"><UiCustomText uiKey="u-de9bff6a0980">Điện thoại</UiCustomText></th><th data-ui-key="u-505e5192ba9e" data-ui-label-default="Combo còn lại"><UiCustomText uiKey="u-505e5192ba9e">Combo còn lại</UiCustomText></th><th data-ui-key="u-7b93e5b93d67"><UiCustomText uiKey="u-7b93e5b93d67">Lịch sử</UiCustomText></th><th data-ui-key="u-e5a014143f4d"><UiCustomText uiKey="u-e5a014143f4d">Sửa / Xóa</UiCustomText></th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td><strong>{item.name || 'Chưa có tên'}</strong></td><td>{item.phone || '—'}</td><td>{(item.combo_purchases || []).reduce((sum, purchase) => sum + Number(purchase.remaining || 0), 0)} vé<div className="spa-customer-combos">{(item.combo_purchases || []).map(purchase => <span key={purchase.id}>{purchase.combo_name} · {purchase.remaining} vé {user?.role === 'admin' && canCustomer('customer_combo_edit') && <button data-ui-key="u-a12a2de812aa" data-ui-label-default="Sửa combo" className="text-button" disabled={busy} onClick={() => { setError(''); setCustomerContext({ customer: item, purchase, mode: 'edit', revision: data.revision }) }}><UiCustomText uiKey="u-a12a2de812aa">Sửa combo</UiCustomText></button>}{canCustomer('customer_combo_delete') && <button data-ui-key="u-3086b628da12" data-ui-label-default="Xóa combo" className="text-button" disabled={busy} onClick={() => { setError(''); setCustomerContext({ customer: item, purchase, mode: 'delete', revision: data.revision }) }}><UiCustomText uiKey="u-3086b628da12">Xóa combo</UiCustomText></button>}</span>)}</div></td><td><button data-ui-key="u-bfbba7a933ee" data-ui-label-default="Lịch sử" className="secondary-button spa-history-button" disabled={busy} onClick={() => history(item)}><History size={14}/><UiCustomText uiKey="u-bfbba7a933ee"> Lịch sử</UiCustomText></button></td><td><UiToolbar data-ui-key="u-be633adfe517" className="spa-actions spa-customer-actions"><button data-ui-key="u-e525b5de0bc2" data-ui-label-default="Sửa" className="secondary-button" disabled={busy || !canEditCustomer} onClick={() => { setError(''); setCustomerContext({ customer: item, mode: 'edit', revision: data.revision }) }}><UiCustomText uiKey="u-e525b5de0bc2">Sửa</UiCustomText></button>{canCustomer('customers_delete') && <button data-ui-key="u-893f89123ab8" data-ui-label-default="Xóa" className="secondary-button danger-button" disabled={busy} onClick={() => { setError(''); setCustomerContext({ customer: item, mode: 'delete', revision: data.revision }) }}><UiCustomText uiKey="u-893f89123ab8">Xóa</UiCustomText></button>}</UiToolbar></td></tr>)}</tbody></table></div>
        : tab === 'services' ? <div className="responsive-data-table spa-fit-table"><table className="spa-services-table" data-ui-key="u-b9e6fddde0bb"><thead><tr><th data-ui-key="u-005528d2970c" aria-label="Sắp xếp"></th><th data-ui-key="u-ed10cafbe927" data-ui-label-default="Dịch vụ / nhóm"><UiCustomText uiKey="u-ed10cafbe927">Dịch vụ / nhóm</UiCustomText></th><th data-ui-key="u-2c0b0a013ddb" data-ui-label-default="Loại dịch vụ"><UiCustomText uiKey="u-2c0b0a013ddb">Loại dịch vụ</UiCustomText></th><th data-ui-key="u-e25bda202b15" data-ui-label-default="Thời lượng / thành phần"><UiCustomText uiKey="u-e25bda202b15">Thời lượng / thành phần</UiCustomText></th><th data-ui-key="u-97d3008b8046" data-ui-label-default="Giá"><UiCustomText uiKey="u-97d3008b8046">Giá</UiCustomText></th><th data-ui-key="u-8443c717885b" data-ui-label-default="Số lượt"><UiCustomText uiKey="u-8443c717885b">Số lượt</UiCustomText></th><th data-ui-key="u-2c5792c8b91d" data-ui-label-default="Admin duyệt bán"><UiCustomText uiKey="u-2c5792c8b91d">Admin duyệt bán</UiCustomText></th><th data-ui-key="u-7948ed0ef0fc" data-ui-label-default="Thao tác"><UiCustomText uiKey="u-7948ed0ef0fc">Thao tác</UiCustomText></th></tr></thead><tbody>{filtered.map((item) => <tr key={`${item.catalog_kind}:${item.id}`} {...dragProps(item)}>
          ...
          <td className="spa-approval-cell">{item.catalog_kind === 'combo' ? <input type="checkbox" aria-label={`Admin duyệt bán ${item.name}`} checked={item.requires_admin_approval === true} disabled={busy} onChange={(event) => toggleComboApproval(item, event.target.checked)}/> : '—'}</td>
          <td><UiToolbar data-ui-key="u-0511a65c1d09" className="spa-actions"><button data-ui-key="u-d867ed44bd2a" data-ui-label-default="Sửa" className="secondary-button" disabled={busy} onClick={() => openEditor(item.catalog_kind, item)}><UiCustomText uiKey="u-d867ed44bd2a">Sửa</UiCustomText></button><button data-ui-key="u-619c7216a02c" data-ui-label-default="Xóa" className="secondary-button danger-button" disabled={busy} onClick={() => remove(item.catalog_kind, item)}><Trash2 size={14}/><UiCustomText uiKey="u-619c7216a02c"> Xóa</UiCustomText></button></UiToolbar></td>
        </tr>)}</tbody></table></div>
          : <div data-ui-key="u-a70d4a992413" className="spa-card-grid spa-settings-list">{filtered.map((item) => <article key={item.id} {...dragProps(item, 'spa-card')}><div data-ui-key="u-9a8c9799ad0f" className="spa-card-heading spa-area-summary"><span className="spa-drag-handle" role="button" tabIndex={busy ? -1 : 0} aria-label={`Kéo để sắp xếp ${item.name}`} title="Giữ và kéo để thay đổi vị trí" onKeyDown={(event) => handleSortKey(event, item)} onPointerDown={(event) => handlePointerDown(event, item)} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={cancelPointerDrag}><GripVertical size={19}/></span><h3>{item.kind === 'room' && !/^(phòng|vip)\s/i.test(item.name) ? `Phòng ${item.name}` : item.name}</h3>{item.kind === 'room' ? <><span>{item.beds.length} giường:</span><ul className="spa-bed-list">{item.beds.map((bed) => <li key={bed.id}>{bed.name}{bed.active === false ? ' · Ngừng sử dụng' : ''}</li>)}</ul></> : <span>{areaLabels[item.kind]}</span>}</div><UiToolbar data-ui-key="u-788f3b562dc9" className="spa-actions"><button data-ui-key="u-e2a2b9759c2e" data-ui-label-default="Sửa" className="secondary-button" disabled={busy} onClick={() => openEditor('area', item)}><UiCustomText uiKey="u-e2a2b9759c2e">Sửa</UiCustomText></button><button data-ui-key="u-bc5d818f12ca" data-ui-label-default="Xóa" className="secondary-button danger-button" disabled={busy} onClick={() => remove('area', item)}><Trash2 size={14}/><UiCustomText uiKey="u-bc5d818f12ca"> Xóa</UiCustomText></button></UiToolbar></article>)}</div>}
    </section>
    ...
    {editor && <Editor title={editTitle} onClose={() => { setEditor(null); setError('') }} busy={busy}>
      ...
      {editor.kind === 'choose-service' ? <ServiceTypePicker onChoose={(kind) => openEditor(kind)}/> : editor.kind === 'history' ? <CustomerHistory key={editor.value.customer.id} value={editor.value} canExport={data?.can_export}/> : <form onSubmit={submit}><fieldset disabled={busy} className="spa-form">
        ...
        {editor.kind === 'area' && <>
          ...
          {form.kind === 'room' && <div className="spa-beds-editor"><h3>Giường trong phòng ({form.beds.length})</h3>{form.beds.map((bed, index) => <div className="spa-bed-row" key={bed.id || index}><input aria-label={`Tên giường ${index + 1}`} required maxLength={100} value={bed.name} onChange={(event) => set('beds', form.beds.map((value, i) => i === index ? { ...value, name: event.target.value } : value))}/><button data-ui-key="u-e34e9941ad62" className="icon-button" type="button" aria-label={`Bỏ giường ${index + 1}`} disabled={form.beds.length <= 1} onClick={() => set('beds', form.beds.filter((_, i) => i !== index))}><Trash2 size={16}/></button></div>)}<button data-ui-key="u-a26c8d320e1b" data-ui-label-default="Thêm giường" className="secondary-button" type="button" disabled={form.beds.length >= 100} onClick={() => { let n = form.beds.length + 1; while (form.beds.some((bed) => bed.name === `Giường ${n}`)) n += 1; set('beds', [...form.beds, { name: `Giường ${n}` }]) }}><Plus size={14}/><UiCustomText uiKey="u-a26c8d320e1b"> Thêm giường</UiCustomText></button></div>}
        </>}
        <div className="spa-editor-footer"><button data-ui-key="u-17305fb9ef60" data-ui-label-default="Hủy" className="secondary-button" type="button" onClick={() => setEditor(null)}><UiCustomText uiKey="u-17305fb9ef60">Hủy</UiCustomText></button><button data-ui-key="u-898c6f045e8a" className="primary-button" type="submit"><Save size={16}/>{busy ? 'Đang lưu…' : editor.existing ? 'Lưu thay đổi' : 'Thêm mới'}</button></div>
      </fieldset></form>}
    </Editor>}
  </div>
}
