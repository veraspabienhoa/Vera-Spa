...
export default function HumanResourcesPage({ user }) {
  ...
  return <div className="feature-page hr-page">
    <div className="page-heading"><div><h1>NHÂN SỰ</h1><p>Quản lý bộ phận và hình thức lương độc lập với phân quyền tài khoản.</p></div><button className="secondary-button" disabled={busy} onClick={() => run(load, 'Đã làm mới.')}>Làm mới</button></div>
    {error && <div className="error-box" role="alert">{error}</div>}{notice && <div className="success-box" role="status">{notice}</div>}
    <section className="panel"><h2>Bộ phận & hình thức lương</h2>
      ...
      <form className="hr-department-form" onSubmit={submit}>
        ...
        <button className="primary-button" disabled={busy || !data}>{editing ? 'Lưu thay đổi' : 'Thêm bộ phận'}</button>
        {editing && <button className="secondary-button" type="button" disabled={busy} onClick={() => { setDraft(empty); setEditing(false) }}>Hủy sửa</button>}
      </form>
      <div className="hr-departments">{departments.map(([code, item]) => <article key={code} className={`hr-department ${item.salary_mode}`}><div><strong>{item.name}</strong><small>{code} · {data.employees.filter(person => person.department === code).length} nhân viên</small></div><span>{modes[item.salary_mode]}</span><div className="hr-actions"><button className="secondary-button" disabled={busy} onClick={() => { setDraft({ code, name: item.name, salary_mode: item.salary_mode }); setEditing(true) }}>Sửa</button><button className="secondary-button danger-button" disabled={busy} onClick={() => { if (window.confirm(`Xóa bộ phận ${item.name}? Bộ phận phải không còn nhân viên.`)) void run(() => veraApi.deleteHrDepartment(code, data.revision), 'Đã xóa bộ phận.') }}>Xóa</button></div></article>)}</div>
    </section>
    <section className="panel"><h2>Phân bộ phận cho nhân viên</h2><p>Ví dụ: bộ phận Thu ngân, quyền Lễ tân. Thay bộ phận không thay đổi quyền tài khoản. Cấu hình mức tiền tại Cấu hình lương; bộ phận Tip dùng Lương KTV.</p>
      <div className="hr-filters"><label>Tìm nhân viên<input type="search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Tên hoặc username…"/></label><label>Bộ phận<select value={departmentFilter} onChange={e => setDepartmentFilter(e.target.value)}><option value="">Tất cả</option>{departments.map(([code, item]) => <option key={code} value={code}>{item.name}</option>)}</select></label></div>
      <div className="hr-people">{people.map(person => <article key={person.username}><div><strong>{person.full_name || person.username}</strong><small>{person.username} · Quyền: {person.role} · {person.employment_status}</small></div><label>Bộ phận<select aria-label={`Bộ phận ${person.username}`} disabled={busy} value={assignments[person.username] || ''} onChange={e => setAssignments({ ...assignments, [person.username]: e.target.value })}><option value="" disabled>Chọn bộ phận</option>{!departments.some(([code]) => code === person.department) && <option value={person.department} disabled>Chưa phân bộ phận</option>}{departments.map(([code, item]) => <option key={code} value={code}>{item.name} · {modes[item.salary_mode]}</option>)}</select></label><button className="primary-button" disabled={busy || assignments[person.username] === person.department || !assignments[person.username]} onClick={() => run(() => veraApi.assignHrDepartment(person.username, assignments[person.username], data.revision), 'Đã chuyển bộ phận; quyền tài khoản được giữ nguyên.')}>Lưu</button></article>)}</div>
