...
const recipientLabel = (id, options) => recipientGroups.find(([key])=>key===id)?.[1] || options.find(person=>person.id===id)?.name || 'Tài khoản không còn hoạt động'
function Recipients({ options, selected, onChange, source }) {
  ...
  return <fieldset className="notification-recipients"><legend>Người nhận · {selected.filter(id=>!id.startsWith('group:')).length} tài khoản · {selected.filter(id=>id.startsWith('group:')).length} nhóm</legend>
    <div className="notification-recipient-groups" role="group" aria-label="Nhóm người nhận">{recipientGroups.map(([key,label])=><label key={key}><input type="checkbox" checked={selected.includes(key)} disabled={key==='group:watchers' && source!=='leave_watch'} onChange={event=>onChange(event.target.checked?[...selected,key]:selected.filter(id=>id!==key))}/>{label}</label>)}</div>
    ...
    <div className="notification-selected">{selected.map(id=><button type="button" key={id} onClick={()=>onChange(selected.filter(value=>value!==id))}>{recipientLabel(id,options)} ×</button>)}</div>
...
export default function NotificationSettingsPage({ user }) {
  ...
  return <div className="notification-settings-page">
    ...
    {editor && draft && <NotificationEditorDialog title={editor==='new'?'Tạo thông báo mới':draft.label} busy={busy} onClose={()=>{setEditor(null);setDraft(null);setError('')}}>
      ...
      <Recipients source={draft.source_key} options={data.recipients || []} selected={draft.recipients} onChange={recipients=>setDraft({...draft,recipients})}/>
