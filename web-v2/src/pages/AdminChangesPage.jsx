...
const labels = { insert: 'Đăng ký mới', update: 'Thay đổi', delete: 'Xóa lịch nghỉ' }
export default function AdminChangesPage() {
  ...
  return <div className="feature-page">
    <div className="page-heading"><div><span className="eyebrow"><Activity size={14} /> Admin</span><h1>THAY ĐỔI HỆ THỐNG</h1><p>Chỉ hiển thị đăng ký lịch nghỉ mới, xóa lịch nghỉ và thay đổi lịch nghỉ.</p></div><button className="secondary-button" onClick={load} disabled={busy}><RefreshCw size={16} className={busy ? 'spin' : ''} /> Làm mới</button></div>
    ...
    <section className="panel audit-list">{data.changes.map((item) => <article key={item.id}><span className={`audit-operation ${item.event_type}`}>{labels[item.event_type] || item.event_type}</span><div><strong>Lịch nghỉ</strong><p>{item.detail || 'Thay đổi lịch nghỉ'}</p></div><time>{formatTime(item.created_at)}</time></article>)}{!data.changes.length && <div className="setup-note">Không có thay đổi lịch nghỉ trong khoảng đã chọn.</div>}</section>
  </div>
}
