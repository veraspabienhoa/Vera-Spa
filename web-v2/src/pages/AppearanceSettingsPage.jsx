...
export default function AppearanceSettingsPage({ user }) {
  ...
  return <div className="feature-page appearance-settings-page">
    ...
    `}</style>
    <div className="appearance-head"><div><div className="topbar-kicker">ADMIN</div><h1>Giao diện</h1><p className="appearance-help">Cấu hình Live Tour độc lập cho Desktop và Mobile. Giá trị 0 nghĩa là giữ kích thước/bố cục CSS mặc định hiện tại.</p></div><div className="appearance-tabs"><button type="button" className={device === 'desktop' ? 'primary-button' : 'secondary-button'} onClick={() => setDevice('desktop')}>Desktop</button><button type="button" className={device === 'mobile' ? 'primary-button' : 'secondary-button'} onClick={() => setDevice('mobile')}>Mobile</button></div></div>
