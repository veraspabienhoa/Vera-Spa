...
// Display-only enhancement. Authorization belongs to the React row controls
// and the API; showing Loại nghỉ must never restrict the Lý do nghỉ editor.
    ...
    loadReasonTypes(controller.signal)
      .then((items) => {
        ...
        for (const item of items) {
          ...
          if (key) next[key] = String(item?.leave_type || '').trim()
        }
        setCatalog(next)
      })
      .catch((error) => {
