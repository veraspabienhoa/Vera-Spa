    ...
    {loading ? <div className="page-loading"><LoaderCircle size={18} className="spin" /> Đang tải lịch…</div> : <div className={`schedule-scroll ${isWeekView ? 'week-view' : ''}`}>
      <table className="schedule-grid">
        ...
        <tbody>{employees.map((employee) => {
          ...
          return <tr key={employee.username} className={isOwn ? 'own-row' : ''}><td className="employee-cell"><div className="employee-name-line"><strong>{systemName(employee)}</strong>{isAdmin && <button type="button" className="system-name-edit" title="Đổi tên hệ thống" onClick={() => void renameSystemName(employee)}><PencilLine size={13}/></button>}</div><span className="employee-role">{DEPARTMENT_INFO[department].label}</span></td>{days.map((date) => {
            const day = isoDate(date)
            const value = { ...emptyCell(), ...(drafts[keyFor(employee.username, day)] || {}) }
        ...
        {department !== 'quanly' && configuredShiftNames.length > 0 && <tfoot>{configuredShiftNames.map((shift) => <tr key={`summary-${shift}`}><td className="summary-label">TỔNG NHÂN SỰ · {shift}</td>{days.map((date) => {
          const day = isoDate(date)
