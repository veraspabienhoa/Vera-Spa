...
function compactCellLabel(value, department) {
  ...
  if (department === 'locker' && value.overtime_shift) {
    label += ` +${value.overtime_shift.replace(/^TC\s+Ca\s+/i, 'TC')}`
  }
...
export default function WorkSchedulePage({ user }) {
  ...
  const isWeekView = rangeMode === 'week' && days.length === 7
  ...
  const pasteCell = async (username, day) => {
    ...
    if (payload.department !== department) {
      setNotice(`Không thể áp dụng dữ liệu ${DEPARTMENT_INFO[payload.department]?.label || payload.department} sang ${DEPARTMENT_INFO[department].label}.`)
      return
    }
    const key = keyFor(username, day)
    setClipboardCell(payload)
  ...
  const applyPasteRange = () => {
    ...
    if (startIndex < 0 || endIndex < startIndex) {
      setNotice('Ngày kết thúc phải từ ngày đang chọn trở đi và nằm trong khoảng đang xem.')
      return
    }
    ...
    setPastePanelOpen(false)
    setNotice(`Đã áp dụng cùng một lịch cho ${targetDays.length} ngày (${targetDays[0]} → ${targetDays[targetDays.length - 1]}). Bấm Lưu lịch để ghi chính thức.`)
  }
      ...
      employees.forEach((employee) => {
        ...
        if (department === 'locker') {
          ...
          shiftNames.forEach((shift) => {
            ...
            if (timeRangesOverlap(spec.start, spec.end, value.overtime_start_time, value.overtime_end_time)) {
              counts[shift].overtime += 1
            }
          })
        }
      })
  ...
  const editorFor = (employee, day, value) => {
    ...
    if (department === 'quanly') {
      return <div className="manager-cell">
        ...
        {value.shift_code === 'Giờ làm' && <div className="manager-time-row">
          <input className="manager-time" type="time" value={value.start_time || ''} disabled={!canEdit} aria-label={`Giờ bắt đầu ${day}`} onChange={(event) => setCell(employee.username, day, 'start_time', event.target.value)} />
          <input className="manager-time" type="time" value={value.end_time || ''} disabled={!canEdit} aria-label={`Giờ kết thúc ${day}`} onChange={(event) => setCell(employee.username, day, 'end_time', event.target.value)} />
        </div>}
      </div>
    }
    ...
    return <div className="schedule-cell">
      ...
      {department === 'locker'
        ...
        : <div className="letan-overtime" title="Tăng ca Lễ tân: để trống cả hai ô nếu không tăng ca">
            <span>TC</span>
            <input className="letan-ot-time" type="time" value={value.overtime_start_time || ''} disabled={!canEdit || !value.shift_code || value.shift_code === 'Nghỉ'} aria-label={`Tăng ca bắt đầu ${day}`} onChange={(event) => setCell(employee.username, day, 'overtime_start_time', event.target.value)} />
            <input className="letan-ot-time" type="time" value={value.overtime_end_time || ''} disabled={!canEdit || !value.shift_code || value.shift_code === 'Nghỉ'} aria-label={`Tăng ca kết thúc ${day}`} onChange={(event) => setCell(employee.username, day, 'overtime_end_time', event.target.value)} />
          </div>}
    </div>
  }
  ...
  return (
    <section className="work-schedule-page">
      ...
      <div className="schedule-head">
        ...
        <div className="schedule-tools">
          <div className="schedule-month-picker">
            <button type="button" className="schedule-icon-button" aria-label="Tháng trước" onClick={() => { const next = moveMonth(month, -1); setMonth(next); setRangeMode('selected_month') }}><ChevronLeft size={17}/></button>
            <CalendarDays size={17}/>
            <input type="month" value={month} onChange={(event) => changeSelectedMonth(event.target.value)} aria-label="Chọn tháng lịch làm việc" />
            <button type="button" className="schedule-icon-button" aria-label="Tháng sau" onClick={() => { const next = moveMonth(month, 1); setMonth(next); setRangeMode('selected_month') }}><ChevronRight size={17}/></button>
          </div>
      ...
      <div className="schedule-filter-bar">
        {RANGE_FILTERS.map(([mode, label]) => <button type="button" key={mode} className={rangeMode === mode ? 'active' : ''} onClick={() => selectRange(mode)}>{label}</button>)}
      </div>
      {rangeMode === 'custom' && <div className="schedule-custom-range">
        <input type="date" value={customStart} onChange={(event) => { const value = event.target.value; setCustomStart(value); if (customEnd < value) setCustomEnd(value) }} aria-label="Ngày bắt đầu tùy chỉnh" />
        <input type="date" min={customStart} value={customEnd} onChange={(event) => setCustomEnd(event.target.value)} aria-label="Ngày kết thúc tùy chỉnh" />
      </div>}
      ...
      {department !== 'quanly' && <div className="schedule-legend">
      ...
      {shiftEditorOpen && canEdit && department !== 'quanly' && <div className="shift-editor">
        <div className="shift-editor-head">
          <div><strong>TẠO / CHỈNH SỬA CA · {DEPARTMENT_INFO[department].label}</strong><div className="schedule-help">Nếu giờ kết thúc nhỏ hơn giờ bắt đầu, hệ thống hiểu là kết thúc vào hôm sau.</div></div>
          <button type="button" className="schedule-config-button" onClick={addShiftDraft}><Plus size={15}/> Thêm ca</button>
        </div>
      ...
      {isWeekView && selectedCell && selectedEmployee && selectedValue && canEdit && <div className="mobile-week-editor">
        <div className="mobile-week-editor-head"><strong>{selectedEmployee.full_name || selectedEmployee.username}</strong> · {selectedCell.day}</div>
        {editorFor(selectedEmployee, selectedCell.day, selectedValue)}
      </div>}

      {loading ? <div className="page-loading"><LoaderCircle size={18} className="spin" /> Đang tải lịch…</div> : (
        <div className={`schedule-scroll ${isWeekView ? 'week-view' : ''}`}>
          <table className="schedule-grid">
            <thead>
              <tr><th className="employee-head" rowSpan="2">Tên nhân viên</th><th className="month-head" colSpan={days.length}>{rangeTitle}</th></tr>
              <tr>{days.map((date) => {
                const day = isoDate(date)
                const classes = [date.getDay() === 0 ? 'sunday' : '', day === todayIso ? 'today' : ''].filter(Boolean).join(' ')
                return <th key={day} className={classes}><div className="weekday-full">{WEEKDAYS[date.getDay()]}</div><div className="weekday-short">{WEEKDAYS_SHORT[date.getDay()]}</div><small>{displayDate(date)}</small></th>
              })}</tr>
            </thead>
              ...
              return <tr key={employee.username} className={isOwn ? 'own-row' : ''}>
                ...
                {days.map((date) => {
                  ...
                  return <td key={day} className={tdClass} tabIndex={0} onFocus={() => setSelectedCell({ username: employee.username, day })} onClick={() => setSelectedCell({ username: employee.username, day })} onKeyDown={(event) => handleCellKeyDown(event, employee.username, day)}>
                    <div className="schedule-cell-editor">{editorFor(employee, day, value)}</div>
                    <div className="mobile-cell-summary">{compactCellLabel(value, department)}</div>
                  </td>
                })}
              </tr>
            })}</tbody>
            {department !== 'quanly' && configuredShiftNames.length > 0 && <tfoot>
              {configuredShiftNames.map((shift) => <tr key={`summary-${shift}`} className="locker-summary-row">
                <td className="summary-label">TỔNG NHÂN SỰ · {shift}</td>
                {days.map((date) => {
                  const day = isoDate(date)
