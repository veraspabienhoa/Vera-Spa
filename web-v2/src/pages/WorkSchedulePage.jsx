  ...
  return <section data-ui-key="u-cf2cd9c589b7" className="combo-employee-card">
    ...
    {canEdit && <div className="combo-sale-fields">
      ...
      <UiToolbar data-ui-key="u-3233d64b529b" className="combo-form-actions">
        ...
        <button data-ui-key="u-852fe066c3ad" type="button" className="schedule-save" disabled={busy} onClick={() => void submit()}>{editingId ? <Save size={15}/> : <Plus size={15}/>} {editingId ? 'Lưu sửa' : 'Thêm'}</button>
      </UiToolbar>
    </div>}
    <div className="schedule-scroll">
      <table data-ui-key="u-980482c59d81" className="combo-sale-table">
        <thead><tr><th data-ui-key="u-42676ce41b00" data-ui-label-default="Ngày bán"><UiCustomText uiKey="u-42676ce41b00">Ngày bán</UiCustomText></th><th data-ui-key="u-a7d189195d70" data-ui-label-default="Tên khách hàng"><UiCustomText uiKey="u-a7d189195d70">Tên khách hàng</UiCustomText></th><th data-ui-key="u-7c4d2012c19f" data-ui-label-default="Số điện thoại"><UiCustomText uiKey="u-7c4d2012c19f">Số điện thoại</UiCustomText></th><th data-ui-key="u-9ebaa94f10a2" data-ui-label-default="Vé combo"><UiCustomText uiKey="u-9ebaa94f10a2">Vé combo</UiCustomText></th><th data-ui-key="u-f30f96190e0b" data-ui-label-default="Ghi chú"><UiCustomText uiKey="u-f30f96190e0b">Ghi chú</UiCustomText></th>{canEdit && <th data-ui-key="u-58b1a768f47b" data-ui-label-default="Thao tác"><UiCustomText uiKey="u-58b1a768f47b">Thao tác</UiCustomText></th>}</tr></thead>
        <tbody>{rows.map((sale) => <tr key={sale.id}>
          ...
          {canEdit && <td><UiToolbar data-ui-key="u-436df9ca21d3" className="combo-row-actions"><button data-ui-key="u-39ee13908bb9" type="button" className="combo-edit" title="Sửa" aria-label={`Sửa combo ${sale.customer_name}`} onClick={() => beginEdit(sale)}><PencilLine size={14}/></button><button data-ui-key="u-cc7c22ac8251" type="button" className="combo-delete" title="Xóa" aria-label={`Xóa combo ${sale.customer_name}`} onClick={() => void onDelete(sale)}><Trash2 size={14}/></button></UiToolbar></td>}
        </tr>)}</tbody>
      </table>
...
export default function WorkSchedulePage({ user }) {
  ...
  if (!availableDepartments.length) return <section data-ui-key="u-2afe292ca0ca" className="work-schedule-page"><div className="warning-box">Tài khoản chưa được cấp quyền Lịch làm việc.</div></section>
  ...
  return <section data-ui-key="u-de60c0ede58d" className="work-schedule-page">
    ...
    <div className="schedule-head">
      ...
      <div className="schedule-tools">
        <div className="schedule-month-picker">
          <button data-ui-key="u-78c03be10e7c" type="button" className="schedule-icon-button" onClick={() => { const next = moveMonth(month, -1); setMonth(next); setRangeMode('selected_month') }}><ChevronLeft size={17}/></button>
    ...
    <div className="schedule-filter-bar">{RANGE_FILTERS.map(([mode, label]) => <button data-ui-key="u-6f098dedecb0" type="button" key={mode} className={rangeMode === mode ? 'active' : ''} onClick={() => selectRange(mode)}>{label}</button>)}</div>
    ...
    {loading ? <div className="page-loading"><LoaderCircle size={18} className="spin" /> Đang tải lịch…</div> : <div className={`schedule-scroll ${isWeekView ? 'week-view' : ''} ${isMonthView ? 'month-view' : ''}`.trim()}>
      <table data-ui-key="u-e0c90c8ff947" className="schedule-grid">
        <thead><tr><th data-ui-key="u-e1888c33b316" data-ui-label-default="Tên nhân viên" className="employee-head" rowSpan="2"><UiCustomText uiKey="u-e1888c33b316">Tên nhân viên</UiCustomText></th><th data-ui-key="u-6e0efd07f097" className="month-head" colSpan={days.length}>{rangeTitle}</th></tr><tr>{days.map((date) => {
          ...
          return <th data-ui-key="u-d6894b16c662" key={day} className={[classes, isEmployeeWorkDay ? 'employee-work-day' : ''].filter(Boolean).join(' ')}><div className="weekday-full">{WEEKDAYS[date.getDay()]}</div><div className="weekday-short">{WEEKDAYS_SHORT[date.getDay()]}</div><small>{isMonthView ? date.getDate() : displayDate(date)}</small></th>
        })}</tr></thead>
        <tbody>{employees.map((employee) => {
          ...
          return <tr key={employee.username} className={isOwn ? 'own-row' : ''}><td className="employee-cell"><div className="employee-name-line"><button data-ui-key="u-bcf9b8f0884f" type="button" className={`employee-highlight-button ${isHighlightedEmployee || isHighlightedByTotal ? 'active' : ''}`.trim()} aria-pressed={isHighlightedEmployee || isHighlightedByTotal} title="Highlight các ngày nhân viên làm việc" onClick={() => { setHighlightedTotal(null); setHighlightedEmployee((current) => current === employee.username ? '' : employee.username) }}><strong>{systemName(employee)}</strong></button>{isAdmin && <button data-ui-key="u-4c2fd6ca7829" type="button" className="system-name-edit" title="Đổi tên hệ thống" onClick={() => void renameSystemName(employee)}><PencilLine size={13}/></button>}</div><span className="employee-role">{DEPARTMENT_INFO[department].label}</span></td>{days.map((date) => {
            const day = isoDate(date)
            const value = { ...emptyCell(), ...(drafts[keyFor(employee.username, day)] || {}) }
        ...
        {department !== 'quanly' && configuredShiftNames.length > 0 && <tfoot>{configuredShiftNames.map((shift) => <tr key={`summary-${shift}`}><td className="summary-label">Tổng NV · {shift}</td>{days.map((date) => {
          ...
          return <td key={`${shift}-${day}`} className={day === todayIso ? 'today' : ''}><button data-ui-key="u-cabe61b7ed0a" type="button" className={`shift-total-cell ${active ? 'active' : ''}`} title={`Highlight nhân viên ${shift} ngày ${displayFullDate(date)}`} onClick={() => { setHighlightedEmployee(''); setHighlightedTotal((current) => current?.day === day && current?.shift === shift ? null : { day, shift }) }}><b>{counts.total}</b><small>{counts.regular} chính + {counts.overtime} TC</small></button></td>
        })}</tr>)}</tfoot>}
      </table>
