...
export default function PayrollPageEnhanced({ user, activeTab = 'calculate', onTabChange }) {
  ...
  return <div className={`feature-page payroll-page payroll-page-enhanced payroll-tab-${activeTab}`}>
    <div className="page-heading"><div><span className="eyebrow"><WalletCards size={14} /> Kỳ 1 · Kỳ 2</span><h1>BẢNG LƯƠNG</h1><p>Tính lương trực tiếp từ tiền TIP nhân viên, quản lý khấu trừ, hoàn thành và lưu lịch sử bảng lương.</p></div><button className="secondary-button" onClick={reload} disabled={isBusy}><RefreshCw size={16} className={busy === 'load' ? 'spin' : ''} /> Làm mới</button></div>
    ...
    {canCalculate && <section className="panel payroll-calculate-panel">
      ...
      <div className="data-toolbar">
        ...
        <button className="primary-button" onClick={calculate} disabled={isBusy}><WalletCards size={16} /> {busy === 'calculate' ? 'Đang tính…' : 'Tính lương từ TIP'}</button>
      </div>
