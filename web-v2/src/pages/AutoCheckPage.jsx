...
export default function AutoCheckPage({ user }) {
  ...
  const runtimeStatus = String(latestRun?.details?.auto_status || '').toUpperCase()
  const runtimeMismatch = connected && runtimeStatus && runtimeStatus !== String(cfg.status || '').toUpperCase()
  ...
  return <section className="page-stack auto-check-page">
    ...
    {runtimeMismatch && <div className="error-box">Tiến trình Auto Check gần nhất đang ở trạng thái {runtimeStatus}, không khớp cấu hình {String(cfg.status || '').toUpperCase()}. Cần cập nhật tiến trình nền lên phiên bản mới trước khi xử lý dữ liệu.</div>}
