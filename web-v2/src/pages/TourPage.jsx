...
export default function TourPage({ user }) {
  ...
  return <div className="feature-page">
    <div className="page-heading"><div><span className="eyebrow"><Compass size={14} /> Vận hành</span><h1>BẢN TUA</h1><p>Countdown cập nhật mỗi 30 giây; file TourVera được đọc lại tối đa mỗi 5 phút.</p></div>{user?.permissions?.tour_refresh && <button className="secondary-button" onClick={() => load(true)} disabled={busy}><RefreshCw size={16} className={busy ? 'spin' : ''} /> Làm mới Bản tua</button>}</div>
    {error && <div className="error-box">{error}</div>}
    {data.countdown_error && <div className="warning-box">Countdown Bản tua: {data.countdown_error}</div>}
    ...
    <section className="panel"><div className="responsive-data-table tour-table"><table><thead><tr>{data.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{data.records.map((item, index) => <tr className={`tour-row-${item._row_style || 'default'}`} key={index}>{data.columns.map((column) => <td key={column}>{String(item[column] ?? '')}</td>)}</tr>)}</tbody></table></div>{!busy && !data.records.length && <div className="setup-note">Bản tua hiện chưa có dữ liệu.</div>}</section>
