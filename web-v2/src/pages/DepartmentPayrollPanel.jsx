...
export default function DepartmentPayrollPanel({ user, settingsOnly = false }) {
  ...
  const totalNet = visibleRows.reduce((sum, row) => sum + Number(row.net_salary || 0), 0)
  const totalAdvance = visibleRows.reduce((sum, row) => sum + Number(row.advance || 0), 0)
  ...
  const exportExcel = () => run('export', async () => {
    const blob = await request('/v2/department-payroll/combined/export.xlsx', { method: 'POST', body: JSON.stringify({ ...payload(), rows: visibleRows }), download: true })
  ...
  return <div className="feature-page department-payroll-page">
    <section data-ui-key="u-fe386b14128e" className="panel department-payroll-panel">
      ...
      {!!rows.length && <>
        ...
        <div className="department-payroll-filters">
          <label>Tìm nhân viên<input type="search" value={payrollSearch} placeholder="Nhập tên hoặc tên đăng nhập…" onChange={event => { setPayrollSearch(event.target.value); setSelected([]) }} /></label>
          <label>Bộ phận<select aria-label="Lọc bộ phận bảng lương" value={departmentFilter} onChange={event => { setDepartmentFilter(event.target.value); setSelected([]) }}><option value="">Tất cả bộ phận</option>{rowDepartments.map(([code, label]) => <option key={code} value={code}>{label}</option>)}</select></label>
          <span role="status">Hiển thị {visibleRows.length}/{rows.length} nhân viên</span>
        </div>
        <div className="department-payroll-summary"><span>Nhân viên theo bộ lọc<strong>{visibleRows.length}</strong></span><span>Tổng ứng lương theo bộ lọc<strong>{money(totalAdvance)}</strong></span><span>Tổng thực nhận theo bộ lọc<strong>{money(totalNet)}</strong></span></div>
        <p className="department-payroll-filter-note">Bộ lọc áp dụng cho bảng, tổng tiền và Excel. Lưu nháp và Hoàn thành vẫn lưu toàn bộ bảng lương.</p>
