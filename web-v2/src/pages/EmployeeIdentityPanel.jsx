import { Camera, Crop, Download, Eye, EyeOff, KeyRound, LoaderCircle, RotateCcw, RotateCw, ShieldCheck, SlidersHorizontal, Trash2, Upload, X } from 'lucide-react'
...
export default function EmployeeIdentityPanel({ username, allowPasswordReset = false, className = '' }) {
  ...
  const resetPassword = () => run('password', async () => {
    ...
    setPassword(''); setConfirmPassword(''); setShowPassword(false); setShowConfirmPassword(false); setNotice({ type: 'success', message: result.message }); return false
  })
  ...
  return <div className={`employee-identity-panel ${className}`}>
    <style>{`
      ...
      .employee-password-reset{display:grid;gap:10px;padding:13px;border:1px solid #eadfcf;border-radius:14px;background:#fffaf2}.employee-password-reset-head{display:flex;gap:8px;align-items:flex-start}.employee-password-reset-head h4{margin:0;font-size:13px}.employee-password-reset-head p{margin:3px 0 0;font-size:11px;color:#776d60;line-height:1.45}.employee-password-reset-grid{display:grid;grid-template-columns:1fr 1fr auto;gap:9px;align-items:end}.employee-password-field{display:grid;gap:5px;min-width:0}.employee-password-field>label{font-size:12px;font-weight:700}.employee-password-control{position:relative;min-width:0}.employee-password-control input{width:100%;min-width:0;padding-right:42px}.employee-password-eye{position:absolute;right:6px;top:50%;transform:translateY(-50%);display:flex;align-items:center;justify-content:center;width:32px;height:32px;min-height:0;padding:0;border:0;border-radius:8px;background:transparent;color:#50635a;cursor:pointer}.employee-password-eye:hover{background:#edf3ef;color:#173d2f}.employee-password-eye:focus-visible{outline:2px solid #1f6845;outline-offset:1px}.employee-identity-notice{padding:9px 11px;border-radius:10px;font-size:12px}.employee-identity-notice.success{background:#edf8f2;color:#17603b}.employee-identity-notice.error{background:#fff1f0;color:#a62a20}
      ...
      @media(max-width:700px){.employee-identity-panel{padding:12px;gap:11px}.employee-identity-grid{grid-template-columns:1fr}.employee-id-preview{height:118px}.employee-password-reset-grid{grid-template-columns:1fr}.employee-password-reset-grid>.employee-password-submit{width:100%}.identity-editor-backdrop{padding:7px}.identity-editor-card,.identity-camera-card{padding:12px;border-radius:14px}.identity-editor-layout{grid-template-columns:1fr}.identity-editor-preview{min-height:220px}.identity-editor-preview canvas{max-height:34vh}.identity-editor-footer{display:grid;grid-template-columns:1fr 1fr}.identity-editor-footer button{width:100%}.identity-camera-landscape{width:min(95vw,680px)}}
    `}</style>
    ...
    {allowPasswordReset && <div className="employee-password-reset"><div className="employee-password-reset-head"><KeyRound size={17}/><div><h4>RESET MẬT KHẨU NHÂN VIÊN</h4><p>Mật khẩu hiện tại không hiển thị trên trình duyệt để bảo vệ tài khoản. Admin có thể đặt mật khẩu mới; nhân viên bắt buộc đổi lại sau lần đăng nhập tiếp theo.</p></div></div><div className="employee-password-reset-grid"><div className="employee-password-field"><label htmlFor="employee-reset-password">Mật khẩu mới</label><div className="employee-password-control"><input id="employee-reset-password" type={showPassword ? 'text' : 'password'} minLength={8} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Tối thiểu 8 ký tự"/><button type="button" className="employee-password-eye" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? 'Ẩn mật khẩu mới' : 'Xem mật khẩu mới'} title={showPassword ? 'Ẩn mật khẩu mới' : 'Xem mật khẩu mới'}>{showPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button></div></div><div className="employee-password-field"><label htmlFor="employee-reset-password-confirm">Xác nhận mật khẩu</label><div className="employee-password-control"><input id="employee-reset-password-confirm" type={showConfirmPassword ? 'text' : 'password'} minLength={8} autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)}/><button type="button" className="employee-password-eye" onClick={() => setShowConfirmPassword((value) => !value)} aria-label={showConfirmPassword ? 'Ẩn xác nhận mật khẩu' : 'Xem xác nhận mật khẩu'} title={showConfirmPassword ? 'Ẩn xác nhận mật khẩu' : 'Xem xác nhận mật khẩu'}>{showConfirmPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button></div></div><button type="button" className="primary-button employee-password-submit" onClick={resetPassword} disabled={busy === 'password'}>{busy === 'password' ? <LoaderCircle className="spin" size={16}/> : <KeyRound size={16}/>} Reset mật khẩu</button></div></div>}
