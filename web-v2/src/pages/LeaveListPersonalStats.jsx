...
const emptySummary = { totalLeave: 0, paid: 0, generated: 0, unpaid: 0, totalPenalty: 0 }
const sumDailyStats = (days = []) => days.reduce((s, day) => ({ totalLeave: s.totalLeave + Number(day.total_leave || 0), paid: s.paid + Number(day.paid || 0), generated: s.generated + Number(day.generated || 0), unpaid: s.unpaid + Number(day.unpaid || 0), totalPenalty: s.totalPenalty + Number(day.total_penalty || 0) }), { ...emptySummary })
...
export default function LeaveListPersonalStats({ user }) {
  ...
  useEffect(() => {
    ...
    const scheduleSync = () => { if (timer) window.clearTimeout(timer); timer = window.setTimeout(syncFromList, 30) }
    syncFromList()
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try { const result = await veraApi.leaveDailyStats(context.start, context.end, context.employee); if (!cancelled) setSummary(sumDailyStats(result.days || [])) }
      catch (err) { if (!cancelled) { setSummary(emptySummary); setError(err.message || 'Không tải được thống kê nghỉ.') } }
  ...
  return createPortal(<>
    ...
    <section className={`leave-list-personal-summary ${isAdmin ? 'admin' : ''}`} aria-live="polite">
      ...
      {!isAdmin && <div className="leave-list-personal-summary-note">{canViewListScope ? 'Thống kê theo Tất cả nhân viên hoặc nhân viên đang lọc. Tiền vi phạm không hiển thị cho tài khoản này.' : 'Thống kê này chỉ tính lịch nghỉ của chính tài khoản đang đăng nhập. Tiền vi phạm không hiển thị cho nhân viên.'}</div>}
