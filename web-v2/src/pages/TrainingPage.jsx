...
import { BarChart3, BellRing, BookOpenCheck, ClipboardCheck, FileDown, ImageDown, Settings2, X } from 'lucide-react'
...
export default function TrainingPage({ user }) {
  ...
  const [reportFilters, setReportFilters] = useState({ q: '', date_from: '', date_to: '', rating: 'all' })
  ...
  const load = async () => { setBusy(true); try { const result = await veraApi.trainingBootstrap(); setData(result); setNotificationRecipients((result.notification_recipients || []).map(x => x.username)); if (!session.employee_username && result.training_students?.[0]) setSession(current => ({ ...current, employee_username: result.training_students[0].username })); if (!reportEmployee && result.employees?.[0]) setReportEmployee(result.employees[0].username) } catch (error) { setNotice({ type: 'error', text: error.message }) } finally { setBusy(false) } }
  ...
  const openReport = async () => { if (!reportEmployee) return; setBusy(true); try { setReport(await veraApi.trainingReport(reportEmployee, { ...reportFilters, evaluator_role: evaluatorRole })); setTab('report') } catch (error) { setNotice({ type: 'error', text: error.message }) } finally { setBusy(false) } }
  ...
  const visibleSessions = (data?.sessions || []).filter(item => evaluatorRole === 'all' || item.evaluator_role === evaluatorRole)
  ...
  return <main className="training-page">
    ...
    {tab === 'sessions' && <section className="training-grid">
      ...
      <div className="training-card"><div className="training-card-heading"><h2>Lịch sử gần đây</h2><label>Người đào tạo<select value={evaluatorRole} onChange={e => setEvaluatorRole(e.target.value)}><option value="all">Tất cả</option><option value="leader">Chỉ hiển thị Leader</option><option value="quanly">Chỉ hiển thị Quản lý</option></select></label></div><div className="training-list">{visibleSessions.map(item => <article key={item.id}><div><strong>{item.employee_name}</strong><small>{formatDate(item.training_date)} · {String(item.start_time).slice(0,5)}–{String(item.end_time).slice(0,5)} · {item.trainer_name}</small></div><b className={`grade grade-${item.skill_grade.replace('+','plus')}`}>{item.skill_grade}</b><p>{item.topic || 'Buổi đào tạo'}</p><p className="muted">Tinh thần: {item.learning_attitude}{item.improvements ? ` · Cần cải thiện: ${item.improvements}` : ''}</p>{permissions.training_session_update && (data?.is_admin || item.trainer_username?.toLowerCase() === user?.employee_username?.toLowerCase()) && <button type="button" className="text-button" onClick={() => { setEditingSessionId(item.id); setSession({ employee_username:item.employee_username, training_date:String(item.training_date).slice(0,10), start_time:String(item.start_time).slice(0,5), end_time:String(item.end_time).slice(0,5), topic:item.topic, learning_attitude:item.learning_attitude, skill_grade:item.skill_grade, strengths:item.strengths, improvements:item.improvements, notes:item.notes }); window.scrollTo({ top:0, behavior:'smooth' }) }}>Sửa nhật ký</button>}</article>)}{!visibleSessions.length && <div className="training-empty">Không có lịch sử phù hợp bộ lọc.</div>}</div></div>
    </section>}
    ...
    {tab === 'report' && <section>
      <div className="training-report-filter advanced">
        <UsernameAutocomplete label="Nhân viên" options={data?.employees || []} value={reportEmployee} onChange={setReportEmployee}/>
        <label>Từ khóa<input value={reportFilters.q} onChange={e => setReportFilters({ ...reportFilters, q:e.target.value })} placeholder="Tên đợt, ghi chú, username…"/></label>
        <label>Từ ngày<VeraDateInput value={reportFilters.date_from} onChange={e => setReportFilters({ ...reportFilters, date_from:e.target.value })}/></label>
        <label>Đến ngày<VeraDateInput value={reportFilters.date_to} min={reportFilters.date_from || undefined} onChange={e => setReportFilters({ ...reportFilters, date_to:e.target.value })}/></label>
        <label>Xếp loại<select value={reportFilters.rating} onChange={e => setReportFilters({ ...reportFilters, rating:e.target.value })}><option value="all">Tất cả</option><option value="excellent">Xuất sắc</option><option value="good">Tốt</option><option value="average">Trung bình</option><option value="weak">Yếu</option></select></label>
        <label>Người đánh giá/đào tạo<select value={evaluatorRole} onChange={e => setEvaluatorRole(e.target.value)}><option value="all">Tất cả</option><option value="leader">Chỉ hiển thị Leader</option><option value="quanly">Chỉ hiển thị Quản lý</option></select></label>
        <button className="primary-button" onClick={openReport}>Tìm lịch sử</button>
      </div>
      {report && <div className="training-grid"><div className="training-card"><h2>Tiến độ kỹ năng</h2><ProgressChart points={report.progress}/></div><div className="training-card"><h2>Năng lực kỳ gần nhất</h2><Radar values={report.latest_radar}/>{report.latest_radar && <p className="center muted">{report.latest_radar.cycle_name}</p>}</div><div className="training-card wide-card"><h2>Lịch sử Đào tạo & Đánh giá <small>({report.history_total})</small></h2><div className="timeline">{report.history?.map(item => <article key={`${item.type}-${item.id}`}><time>{formatDate(item.date)}</time><div><span className={`history-kind ${item.type}`}>{item.type === 'daily' ? 'Đào tạo hằng ngày' : 'Đánh giá tổng hợp'}</span><strong>{item.title}</strong><p>Người thực hiện: {item.evaluator_name} · Xếp loại: {item.rating_label}</p>{item.type === 'daily' ? <p className="muted">Kỹ năng: {item.detail.skill_grade} · Tinh thần: {item.detail.learning_attitude}{item.detail.improvements ? ` · Cần cải thiện: ${item.detail.improvements}` : ''}</p> : <><p className="muted">Tay nghề {item.detail.craft_score}/5 · Giao tiếp {item.detail.communication_score}/5 · Thái độ {item.detail.attitude_score}/5</p><div className="history-export"><button type="button" onClick={() => exportEvaluation(item.id,'pdf')}><FileDown size={14}/> PDF</button><button type="button" onClick={() => exportEvaluation(item.id,'png')}><ImageDown size={14}/> Ảnh PNG</button></div></>}</div></article>)}{!report.history?.length && <div className="training-empty">Chưa có lịch sử phù hợp bộ lọc.</div>}</div></div></div>}
    </section>}
    ...
    {tab === 'admin' && <section className="training-grid"><div className="training-card"><h2>Tài khoản nhận thông báo</h2><p className="muted">Admin và nhân viên được đánh giá luôn nhận thông báo. Chọn thêm các tài khoản cần nhận mọi kết quả hoàn thành.</p><div className="check-list">{data?.people?.filter(x => x.role !== 'admin').map(x => <label key={x.username}><input type="checkbox" checked={notificationRecipients.includes(x.username)} onChange={() => setNotificationRecipients(current => current.includes(x.username) ? current.filter(v => v !== x.username) : [...current,x.username])}/>{x.full_name} · {x.role}</label>)}</div><button className="primary-button" disabled={busy} onClick={saveNotificationRecipients}>Lưu người nhận thông báo</button></div>
