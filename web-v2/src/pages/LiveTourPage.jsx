          ...
          <div className="tour-room-grid" style={{ '--room-columns': Math.max(1, Math.ceil(displayedRooms.length / 2)) }}>
            {displayedRooms.map((room) => {
              ...
              return <div data-room-density={Math.min(records.length, 6)} className={`tour-room-card ${isVipArea(room) ? 'vip' : 'standard'} state-${roomState(record, available, clockMs)} ${hasPrivateService ? 'has-private-service' : records.length ? 'has-standard-service' : ''} ${selectedRoomKey === key ? 'selected' : ''} ${searchedRoomKeys.has(key) ? 'search-match' : ''}`.trim()} key={key}>
                ...
                <div className={`tour-room-countdown ${/^(Còn|Trễ) \d+:\d+$/.test(countdown) ? 'tour-room-countdown-timed' : ''}`}><Clock3 size={16}/><span className={countdown === 'Đã hết giờ' ? 'tour-room-countdown-expired' : countdown === 'Đang trống' ? 'tour-room-countdown-empty' : /^(Còn|Đang chờ)/.test(countdown) ? 'tour-room-countdown-label' : undefined}>{/^(Còn|Trễ) (\d+):(\d+)$/.test(countdown) ? <><span className="countdown-prefix">{countdown.split(' ')[0]} </span><span className="countdown-hours">{countdown.match(/\d+/)?.[0]}<small>giờ</small></span><span className="countdown-colon">:</span><span className="countdown-minutes">{countdown.match(/:(\d+)/)?.[1]}<small>phút</small></span></> : countdown}</span></div>
