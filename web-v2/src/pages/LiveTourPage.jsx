  ...
  return <div className="feature-page tour-page live-tour-page">
    ...
    <section className={`panel live-tour-operator live-tour-workspace ${privilegedLiveTourRole ? '' : 'live-tour-management-hidden'}`} ref={workspaceRef}>
      ...
      {activePanel === 'customers' && canCustomers && <div className="live-tour-panel-body">
        <div className="live-tour-panel-toolbar"><h2>KHÁCH HÀNG</h2><div className="live-tour-panel-toolbar-actions"><button type="button" className="primary-button" disabled={!canPayment} onClick={() => openModal('combo_purchase', { rowIds: [] })}><Plus size={13}/> Mua combo cho khách hàng</button>{canImportCombo && <button type="button" className="secondary-button" onClick={() => openModal('combo_import')}>Nhập combo</button>}<button type="button" className="secondary-button" disabled={!canExportKind('customers')} onClick={() => exportData('customers')}><Download size={13}/> Xuất khách hàng</button></div></div>
