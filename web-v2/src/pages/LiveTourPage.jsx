...
export default function LiveTourPage({ user, navigationToggle = null }) {
  ...
  return <div className="feature-page tour-page live-tour-page">
    <style>{`
      ...
      .live-tour-page .tour-room-card.state-green:not(.vip){--room-segment:#34791a;--room-border:#4a9627;--room-glow:rgba(74,150,39,.34);background:var(--tour-row-green)}.live-tour-page .tour-room-card.state-yellow:not(.vip){--room-segment:#967018;--room-border:#bb8b20;--room-glow:rgba(187,139,32,.32);background:var(--tour-row-yellow)}.live-tour-page .tour-room-card.state-red:not(.vip){--room-segment:#a9212b;--room-border:#ce3541;--room-glow:rgba(206,53,65,.31);background:var(--tour-row-red)}.live-tour-page .tour-room-card.state-break:not(.vip){--room-segment:#a75a17;--room-border:#cf792b;--room-glow:rgba(207,121,43,.3);background:var(--tour-row-break)}.live-tour-page .tour-room-card.state-waiting:not(.vip){--room-segment:#684090;--room-border:#8658b2;--room-glow:rgba(134,88,178,.33);color:#3f245d;background:var(--tour-row-waiting)}.live-tour-page .tour-room-card.state-idle:not(.vip){--room-segment:#587369;--room-border:#708f83;--room-glow:rgba(112,143,131,.26);background:var(--tour-row-idle)}.live-tour-page .tour-room-card.state-leave,.live-tour-page .tour-room-card.state-work,.live-tour-page .tour-room-card.state-default,.live-tour-page .tour-room-card.state-blank{background:#fff}.live-tour-page .tour-room-card.state-leave{color:#8a8a8a}
    ...
    <section className="panel tour-table-panel tour-records-panel">
      <div className="responsive-data-table tour-table" tabIndex="0" aria-label="Danh sách Live Tour"><table><thead><tr><th className="live-tour-select-col"><input type="checkbox" checked={allDisplayedSelected} onChange={toggleDisplayed} aria-label="Chọn tất cả nhân viên đang hiển thị"/></th>{columns.map((column) => <Fragment key={column}><th className={columnClass(column)}>{column}</th>{column === employeeColumn && canOperate && <th className="live-tour-actions-col">Thao tác</th>}</Fragment>)}</tr></thead><tbody>{displayedRecords.map((item, index) => {
        ...
        return <tr className={rowClass(item, selectedIds.has(id), data.payment_settings?.shift_ready_times, clockMs)} key={id} onClick={(event) => { if (!event.target.closest('button,input,a,select')) toggleRow(id) }}><td className="live-tour-select-col"><input type="checkbox" checked={selectedIds.has(id)} onChange={() => toggleRow(id)} aria-label={`Chọn ${cellValue(item, employeeColumn)}`}/></td>{columns.map((column) => <Fragment key={column}><td className={columnClass(column)}>{column === employeeColumn ? <><button type="button" className="text-button" title={String(item[column] ?? '')} disabled={!canOperate && !canPayment && !canBook} onClick={() => openEmployeeBooking(item)}>{String(item[column] ?? '')}</button>{canReorder && selectedIds.size === 1 && selectedIds.has(id) && <div className="tour-inline-reorder" onClick={(event) => event.stopPropagation()}><input type="number" aria-label={`STT mới của ${cellValue(item, employeeColumn)}`} placeholder="STT" min="1" step="1" value={targetPosition} disabled={Boolean(actionBusy)} onChange={(event) => setTargetPosition(event.target.value)}/><button type="button" className="secondary-button" disabled={Boolean(actionBusy) || !Number.isInteger(Number(targetPosition)) || Number(targetPosition) < 1} onClick={() => runSingleSelected('admin_reorder', { direction: 'position', position: Number(targetPosition) })}>Đổi STT</button></div>}</> : column === appointmentColumn && canEditAppointment ? appointmentEditor(item) : column === sttColumn(columns) ? String(item[column] ?? '') : (column === statusColumn && hasGroup(item, 'doing') ? 'Thực hiện' : String(breakCellValue(item, column, clockMs)))}</td>{column === employeeColumn && canOperate && <td className="live-tour-actions-col">{employeeServiceActions(item)}</td>}</Fragment>)}</tr>
      })}</tbody></table></div>
    ...
    <section className="panel live-tour-operator live-tour-workspace" ref={workspaceRef}>
      ...
      {activePanel === 'invoices' && canPaidInvoiceView && <div className="live-tour-panel-body">
        ...
        <div className="live-tour-card-grid">{visibleInvoices.slice().reverse().map((invoice) => <article className="live-tour-data-card" key={invoice.id}>
          <strong>{invoice.bill_no} · {invoice.customer_name || 'Khách lẻ'}</strong><span>{formatVeraDateTime(invoice.effective_at || invoice.created_at)} · {invoice.payment_method}</span><strong>{formatMoney(invoice.total)}</strong>
