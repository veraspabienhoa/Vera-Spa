...
export default function LiveTourPage({ user, navigationToggle = null }) {
  ...
  return <div className="feature-page tour-page live-tour-page">
    <style>{`
      .live-tour-page{gap:0}.page-wrap.live-tour-page-wrap{padding-top:0;padding-bottom:0}.live-tour-page>.setup-note{padding:6px 9px;font-size:9px}
      .live-tour-board{min-width:0}.live-tour-board>.tour-records-panel{margin-top:0}
      .live-tour-page .tour-board-top{position:static;display:grid;gap:0;background:var(--paper,#f7faf8)}
      .live-tour-page .tour-topbar{display:flex;flex-wrap:wrap;align-items:center;gap:0 6px}
      ...
      @media(max-width:640px){
        .live-tour-page .tour-topbar{gap:0 4px}.live-tour-page .tour-heading-title h1{font-size:13px}.live-tour-page .tour-records-panel .tour-table{max-height:none;height:auto;overflow-x:hidden;overflow-y:hidden}
        ...
        .live-tour-page .tour-room-segment-button{padding:0 5px}.live-tour-page .tour-room-segment-button svg{width:12px;height:12px}.live-tour-page .tour-room-segment-button span{font-size:9px}.live-tour-page .tour-room-segment-button small{font-size:8px}
        .live-tour-page .tour-table-panel{padding:0;border-radius:4px;box-shadow:none}.live-tour-page .tour-quick-tools{display:grid;grid-template-columns:minmax(0,1fr);gap:0;margin:0}.live-tour-page .tour-employee-search{min-width:0;max-width:none}.live-tour-page .tour-employee-search input{min-width:0;height:29px;padding:5px 5px 5px 27px;font-size:8px}.live-tour-page .tour-employee-search svg{left:8px;width:14px}.live-tour-selection-summary{font-size:8px}.live-tour-select-col{width:24px;min-width:24px}.live-tour-select-col input{width:12px;height:12px}
        .live-tour-page .tour-room-panel{padding:0}.live-tour-page .tour-room-panel-head{gap:4px;margin-bottom:0}.live-tour-page .tour-room-panel-title{font-size:9px}.live-tour-page .tour-room-panel-head>small{font-size:11px}.live-tour-page .tour-room-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:0 3px}.live-tour-page .tour-room-card{min-height:46px;padding:2px}.live-tour-page .tour-room-card.vip{padding:0}.live-tour-page .tour-room-card-head strong{font-size:8px}.live-tour-page .tour-room-type{padding:0 2px;font-size:4px}.live-tour-page .tour-room-countdown{font-size:9px}.live-tour-page .tour-room-countdown svg{width:10px;height:10px}.live-tour-page .tour-room-meta{font-size:6px}.live-tour-page .tour-room-detail-row{grid-template-columns:minmax(75px,.55fr) minmax(0,1fr);gap:4px;padding:4px;font-size:7px}
      }
    ...
    <div className="tour-board-top">
      ...
      <section className="panel tour-table-panel tour-room-table-panel">
        <div className={`tour-room-panel ${roomSegment}`}>
          <div className="tour-room-panel-head"><div className="tour-room-panel-title">{roomSegment === 'vip' ? <Crown size={16}/> : roomSegment === 'standard' ? <DoorOpen size={16}/> : <LayoutGrid size={16}/>} {roomSegment === 'vip' ? 'Phòng VIP' : roomSegment === 'standard' ? 'Phòng Standard' : 'Tất cả phòng'}</div>
            <div className="tour-room-segment-buttons" role="group" aria-label="Chọn phân khúc phòng">
              <button type="button" className={`tour-room-segment-button all ${roomSegment === 'all' ? 'active' : ''}`} onClick={() => { setRoomSegment('all'); setSelectedRoomKey('') }} aria-pressed={roomSegment === 'all'}><LayoutGrid size={18}/><span>TẤT CẢ PHÒNG</span><small>{roomCatalog.length} phòng</small></button>
              <button type="button" className={`tour-room-segment-button standard ${roomSegment === 'standard' ? 'active' : ''}`} onClick={() => { setRoomSegment('standard'); setSelectedRoomKey('') }} aria-pressed={roomSegment === 'standard'}><DoorOpen size={20}/><span>STANDARD ROOM</span><small>{standardRooms.length} phòng</small></button>
            ...
            <small>{displayedRooms.filter((room) => availableRoomKeys.has(areaKey(room))).length} phòng đang trống</small>
          </div>
      ...
      <form onSubmit={submitModal}>
        <div className="live-tour-form-grid">
          {['checkout', 'quick_checkout'].includes(modal.kind) && <>
            ...
            <label className="live-tour-field"><span>Khách hàng</span><input value={form.customer_name} readOnly={!canCustomers || Boolean(form.customer_id)} onChange={(event) => setForm((current) => ({ ...current, customer_id: '', customer_name: event.target.value, combo_purchase_id: '', payment_method: current.payment_method === 'COMBO' ? 'TIỀN MẶT' : current.payment_method }))} placeholder="Tìm hoặc nhập khách mới"/></label>
            <label className="live-tour-field"><span>Điện thoại</span><input type="tel" value={form.phone} readOnly={!canCustomers || Boolean(form.customer_id)} onChange={(event) => setForm((current) => ({ ...current, customer_id: '', phone: event.target.value, combo_purchase_id: '', payment_method: current.payment_method === 'COMBO' ? 'TIỀN MẶT' : current.payment_method }))} placeholder="Tìm theo số điện thoại"/></label>
            ...
            <label className="live-tour-field"><span>Cách nhập TIP</span><select value={form.tip_mode} onChange={(event) => setForm((current) => ({ ...current, tip_mode: event.target.value }))}><option value="manual">Nhập tiền TIP</option><option value="cards">Chọn thẻ tiền TIP</option></select></label>
            {form.tip_mode === 'manual' ? <label className="live-tour-field"><span>Tiền TIP</span><input type="number" min="0" max="10000000000" step="1" value={form.tip} onChange={(event) => setForm((current) => ({ ...current, tip: event.target.value }))}/></label> : <div className="wide"><div className="tour-tip-cards">{asArray(data.payment_settings?.tip_cards).map((card) => <button type="button" className="secondary-button" key={card.id} aria-pressed={form.tip_card_ids.includes(card.id)} onClick={() => setForm((current) => ({ ...current, tip_card_ids: current.tip_card_ids.includes(card.id) ? current.tip_card_ids.filter((id) => id !== card.id) : [...current.tip_card_ids, card.id] }))}>{card.name} · {formatMoney(card.amount)}</button>)}</div><p>Tổng TIP: <strong>{formatMoney(checkoutTipPreview)}</strong></p></div>}
