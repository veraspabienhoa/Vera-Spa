  ...
  return <div className="feature-page tour-page live-tour-page">
    ...
    <div className="tour-board-top">
      ...
      <div className="tour-topbar">
        {navigationToggle}
        <div className="tour-heading-title"><h1>LIVE TOUR</h1><span className="live-tour-status">TRỰC TIẾP</span><div className="live-tour-weekly-shift"><button type="button" className="live-tour-weekly-shift-button" onClick={() => setWeeklyShiftOpen(current => !current)} aria-expanded={weeklyShiftOpen}><span className="weekly-desktop">CA TUẦN NÀY ①</span><span className="weekly-mobile">Ca tuần ①</span></button>{weeklyShiftOpen && <div className="live-tour-weekly-shift-popover"><strong>Ca tuần này · {weeklyShiftPlan.range}</strong>{['Ca 1','Ca 2'].map(shift => <div key={shift}><b>{shift}</b>{weeklyShiftPlan.rows.filter(row => row.current === shift).map(row => <span key={row.id || row.name}>{row.name} — {row.current} → tuần sau <strong>{row.next}</strong></span>)}</div>)}<div><b>Cố định / khác</b>{weeklyShiftPlan.rows.filter(row => !['Ca 1','Ca 2'].includes(row.current)).map(row => <span key={row.id || row.name}>{row.name} — {row.current}</span>)}</div></div>}</div></div>
        ...
        {(!isAdmin || adminControlsVisible) && <section id="live-tour-admin-controls" className={`panel live-tour-operator live-tour-controls ${privilegedLiveTourRole ? '' : 'live-tour-management-hidden'}`} aria-label="Điều khiển">
          <div className="live-tour-controls-grid">
            <div className="live-tour-controls-actions" role="group" aria-label="Bảng điều khiển Live Tour">
              ...
              <button type="button" className="secondary-button live-tour-mobile-reorder-hidden" disabled={!canReorder || selectedIds.size !== 1 || Boolean(actionBusy)} onClick={() => runSingleSelected('admin_reorder', { direction: 'top', steps: 1 })}>Lên đầu</button>
              ...
              <button type="button" className="secondary-button live-tour-mobile-reorder-hidden" disabled={!canReorder || selectedIds.size !== 1 || Boolean(actionBusy)} onClick={() => runSingleSelected('admin_reorder', { direction: 'up', steps: Number(reorderSteps) })}>Lên {reorderSteps}</button>
    ...
    <section className={`panel live-tour-operator live-tour-workspace ${privilegedLiveTourRole ? '' : 'live-tour-management-hidden'}`} ref={workspaceRef}>
      ...
      {activePanel === 'pending' && canPending && <div className="live-tour-panel-body" id="live-tour-pending-panel" role="tabpanel" aria-label="Hóa đơn chờ thanh toán">
        <div className="live-tour-panel-toolbar"><h2>HÓA ĐƠN CHỜ THANH TOÁN <span className="live-tour-invoice-count" aria-label={`${pendingPayments.length} hóa đơn chờ thanh toán`}>{pendingPayments.length}</span></h2><div className="live-tour-panel-toolbar-actions"><button type="button" className="secondary-button" disabled={!canExportKind('pending')} onClick={() => exportData('pending')}><Download size={13}/> Xuất chờ thanh toán</button></div></div>
        {pendingPayments.length ? <div className="live-tour-card-grid">{pendingPayments.map((item, index) => {
          ...
          return <article className="live-tour-data-card" key={itemId(item, index)}>
            ...
            <div className="live-tour-card-actions"><button type="button" className="primary-button" disabled={!canPayment || Boolean(actionBusy)} onClick={() => openModal('checkout', { item, rowIds: [], defaults: { pending_id: id } })}>Thanh toán</button>
      ...
      {activePanel === 'invoices' && canPaidInvoiceView && <div className="live-tour-panel-body">
        <div className="live-tour-panel-toolbar"><h2>HÓA ĐƠN ĐÃ THANH TOÁN <span className="live-tour-invoice-count" aria-label={`${visibleInvoices.length} hóa đơn đã thanh toán`}>{visibleInvoices.length}</span></h2></div>
