  ...
  return <div className="feature-page tour-page live-tour-page">
    <style>{`
      ...
      @media(max-width:640px){
        ...
        .live-tour-page .tour-room-panel{padding:0}.live-tour-page .tour-room-panel-head{gap:4px;margin-bottom:0}.live-tour-page .tour-room-panel-title{font-size:9px}.live-tour-page .tour-room-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:4px 3px}.live-tour-page .tour-room-card{min-height:59.8px;padding:2px}.live-tour-page .tour-room-card.vip{padding:0}.live-tour-page .tour-room-card-head strong{font-size:8px}.live-tour-page .tour-room-type{padding:0 2px;font-size:4px}.live-tour-page .tour-room-countdown{font-size:9px}.live-tour-page .tour-room-countdown svg{width:10px;height:10px}.live-tour-page .tour-room-meta{font-size:6px}.live-tour-page .tour-room-detail-row{grid-template-columns:minmax(75px,.55fr) minmax(0,1fr);gap:4px;padding:4px;font-size:7px}
      }
          ...
          <div className="tour-room-grid">
            {displayedRooms.map((room) => {
              ...
              return <div className={`tour-room-card ${isVipArea(room) ? 'vip' : 'standard'} state-${roomState(record, available, clockMs)} ${hasPrivateService ? 'has-private-service' : records.length ? 'has-standard-service' : ''} ${selectedRoomKey === key ? 'selected' : ''} ${searchedRoomKeys.has(key) ? 'search-match' : ''}`.trim()} key={key}>
                ...
                {records.length > 0 && <div className="tour-room-services">{records.map((item, index) => <div key={recordId(item, index)}>{cellValue(item, roomColumn)}: {cellValue(item, serviceColumn)}</div>)}</div>}
          ...
          {['checkout', 'quick_checkout'].includes(modal.kind) && <>
            {modal.kind === 'quick_checkout' && <>
              ...
              {manualQuickBooking && <>
                ...
                <LiveTourSearchSelect label="Dịch vụ" showAllOptions required value={form.service_id} options={quickBookingServices.map((service) => ({ value: service.id, label: service.name, detail: formatMoney(service.price) }))} onChange={(id) => setForm((current) => ({ ...current, service_id: id }))}/>
