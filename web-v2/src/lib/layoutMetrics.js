export function readLayoutMetrics(element) {
  ...
  return { width: Math.round(rect.width), height: Math.round(rect.height), rawFont: css.fontFamily,
    appearance: { font_size: number(css.fontSize), font_family: font.includes('segoe') ? 'segoe' : /roboto|arial/.test(font) ? 'roboto' : /georgia/.test(font) ? 'serif' : 'system', font_weight: css.fontWeight === 'bold' ? 700 : number(css.fontWeight) || 400, radius: number(css.borderTopLeftRadius), border_width: number(css.borderTopWidth), padding_x: number(css.paddingLeft), padding_y: number(css.paddingTop), normal: { text: color(css.color), background: background || '#ffffff', border: color(css.borderTopColor) } } }
}
