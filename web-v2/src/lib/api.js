...
async function binaryResponse(path, options = {}) {
  if (!apiBase) throw new Error('Python API V2 chưa được cấu hình.')
  ...
  if (!response) throw new Error(`Không tải được dữ liệu sau 2 lần thử. (${lastError?.message || 'Lỗi mạng'})`)
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}))
...
async function download(path, fallbackName, options = {}) {
  const response = await binaryResponse(path, options)
