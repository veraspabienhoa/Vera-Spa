...
export default function LiveTourServiceActions({ target, room = false, waiting = 0, doing = 0, canStart = true, busy, onStart, onFinish }) {
  const scope = room ? `cả phòng ${target}` : target
  return <div className={`live-tour-service-actions${room ? ' room-actions' : ''}`}>
    {canStart && waiting > 0 && <button type="button" className="live-tour-start-button" disabled={busy} onClick={onStart}
      aria-label={`Thực hiện ${scope}`} title={`Thực hiện ${scope} · ${waiting} nhân viên đang chờ`}>Thực hiện{room && <small>{waiting}</small>}</button>}
    {doing > 0 && <button type="button" className="live-tour-finish-button" disabled={busy} onClick={onFinish}
