...
function LiveTourMultiBookingDialog({ data, context, canBook, canCustomers, canSharePrivateRoom, busy, error, onAction, onClose, onCustomerSearch }) {
  ...
  const customerOptions = (data.customers || []).map((row) => ({ value: row.id, label: row.name, detail: row.phone, displayLabel: [row.name, row.phone].filter(Boolean).join(' - '), badge: customerTicketLabel(row) }))
...
export default function LiveTourBookingDialog({ data, context, canAdmin, canOperate, canBook, canCustomers, canPayment, canSharePrivateRoom, busy, error, onAction, onClose, onCheckout, onCustomerSearch }) {
  ...
  const bookingClock = useBookingClock()
  const employeeOptions = bookingEmployees(employees, bookingClock, data.booking_settings?.employee_available_minutes, data.records).map((row) => {
