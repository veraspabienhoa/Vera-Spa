...
export default function LayoutDesigner({ user, page, open = false, onClose, initialTab }) {
  ...
  return <>
    ...
    {admin && open && <aside className={`layout-designer ${['rooms','columns','history'].includes(tab) ? 'layout-designer-wide' : ''}`} aria-label="Giao diện Admin">
      ...
      <div className="layout-center-tabs" role="tablist" aria-label="Giao diện">{[['order','Sắp xếp'],['buttons','Nút bấm'],['labels','Tên hiển thị'],['style','Hiệu ứng'],['columns','Bảng & cột'],['rooms','Phòng Live Tour'],['history','Lịch sử']].map(([key,label]) => <button type="button" role="tab" key={key} aria-selected={tab === key} onClick={() => chooseTab(key)}>{label}</button>)}</div>
      ...
      {['order','buttons','labels','style'].includes(tab) && <>
