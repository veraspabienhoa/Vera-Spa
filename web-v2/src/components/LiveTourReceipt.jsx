import { receiptNumber } from '../lib/paymentPresentation'
...
export default function LiveTourReceipt({ invoice, autoPrint, onClose, paymentSettings }) {
  ...
  return createPortal(<div id="live-tour-receipt" className="tour-receipt-backdrop"><section ref={dialogRef} tabIndex="-1" className="tour-receipt" role="dialog" aria-modal="true" aria-label={`Hóa đơn ${receiptNumber(invoice.bill_no)}`}>
    ...
    <h2><span className="vera-receipt-brand">VERA</span> SPA</h2><p className="vera-receipt-address">193 Trương Định, Tam Hiệp, Đồng Nai</p><h3>HÓA ĐƠN DỊCH VỤ</h3><p>{receiptNumber(invoice.bill_no)}</p><p>{invoice.effective_at ? new Date(invoice.effective_at).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' }) : invoice.created_at}</p>
    ...
    <p>Thanh toán: {invoice.payment_method}</p>{invoice.combo_units > 0 && <p>Đã trừ combo: {invoice.combo_units} lượt/vé</p>}{invoice.note && <p>Ghi chú: {invoice.note}</p>}<LiveTourPaymentQr bank={paymentSettings?.bank} amount={Number(invoice.total)} reference={invoice.bill_no}/><p className="tour-receipt-thanks">Cảm ơn quý khách!</p>
