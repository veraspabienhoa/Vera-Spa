...
export default function PaymentCustomerScreen({ bank, amount, reference, settings }) {
  ...
  return <div className="customer-screen-control"><button type="button" className="secondary-button" onClick={open}>Mở màn hình QR cho khách</button><small>Có thể kéo cửa sổ sang màn hình phụ; trên điện thoại, mã mở trong tab riêng.</small>{error && <p role="alert">{error}</p>}
