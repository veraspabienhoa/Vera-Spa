...
test('Admin drags within a group, saves desktop only, and another user receives the layout without edit controls', async () => {
  ...
  try {
    ...
    assert.deepEqual([...topbar.querySelectorAll('button')].map(node=>node.textContent), ['Tùy chỉnh','Tạm dừng','Di chuyển','Vị trí'])
    assert.equal(document.querySelector('[role="tab"][aria-selected="true"]').textContent,'Thành phần')
