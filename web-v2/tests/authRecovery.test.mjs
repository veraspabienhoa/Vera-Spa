...
const built = await build({ entryPoints: ['src/App.jsx'], bundle: true, write: false, platform: 'node', format: 'cjs', jsx: 'automatic', external: ['react', 'react/jsx-runtime'], plugins: [{ name: 'fixtures', setup(b) {
  b.onResolve({ filter: /^\.\// }, args => args.kind === 'entry-point' || /\/(usePageRefresh|sharedRead|pageModuleLoader|recoverablePage|PageErrorBoundary)(\.js)?$/.test(args.path) ? undefined : ({ path: args.path, namespace: 'fixture' }))
