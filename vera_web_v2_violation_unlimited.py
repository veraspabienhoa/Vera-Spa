    ...
    def policy_group(conn, reason: str) -> str:
        try:
            ...
            if "co phep" in type_key:
                return "co_phep"
            # A non-empty canonical Loại nghỉ is authoritative.  In particular,
            # Vi phạm must not inherit KHÔNG phép merely from words in the reason.
            if type_key:
                return ""
        except Exception:
            pass
        # Compatibility fallback only for legacy policy rows without Loại nghỉ.
