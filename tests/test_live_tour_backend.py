...
def test_operator_capabilities_allow_hidden_employee_recovery_but_view_only_does_not():
    ...
    operator = live._state_response(
        state, 3, NOW, include_hidden=True, can_operate=True, can_sync=True,
    )
    assert {item["employee_id"] for item in operator["records"]} == {"e1", "e2"}
    assert operator["capabilities"]["operate"] is True
    assert operator["capabilities"]["sync"] is True
...
def test_manual_merge_authorizes_before_downloading_current_tour(monkeypatch):
    app = FastAPI()
    downloads = []

    def denied(_conn, _identity, _feature):
        raise HTTPException(403, "Không có quyền")

    live.install_live_tour_routes(
        app, engine_instance=RouteEngine, current_identity=lambda: RouteIdentity(),
        require_feature=denied, feature_allowed=lambda *_args: False,
        identity_type=RouteIdentity,
    )
    endpoint = next(
        route.endpoint for route in app.routes
    ...
    monkeypatch.setattr(live, "_download_current_tour", lambda: downloads.append("called"))

    with pytest.raises(HTTPException) as error:
        endpoint(
            live.LiveTourAction(
                action="merge_current_tour", idempotency_key="request-key-1", payload={},
            ),
            RouteIdentity(),
        )
...
def test_sync_leaves_authorizes_before_io_and_replay_skips_io(monkeypatch):
    denied_app = FastAPI()
    checked = []
    sync_calls = []

    def denied(_conn, _identity, feature):
        checked.append(feature)
        raise HTTPException(403, "Không có quyền")

    live.install_live_tour_routes(
        denied_app, engine_instance=RouteEngine, current_identity=lambda: RouteIdentity(),
        require_feature=denied, feature_allowed=lambda *_args: False,
        identity_type=RouteIdentity, google_client=lambda: object(), leave_sheet_id="leave-sheet",
    )
    denied_endpoint = next(
        route.endpoint for route in denied_app.routes
        if getattr(route, "path", "") == "/v2/live-tour/action"
    )
    monkeypatch.setattr(live, "_run_leave_sync", lambda **_kwargs: sync_calls.append("called"))
    body = live.LiveTourAction(
        action="sync_leaves", idempotency_key="sync-request-1", payload={"sync_action": "sync_all"},
    )

    with pytest.raises(HTTPException) as denied_error:
        denied_endpoint(body, RouteIdentity())

    assert denied_error.value.status_code == 403
    assert checked == ["tour_leave_sync"]
    assert sync_calls == []

    replay_app = FastAPI()
    replay_state = live._empty_state(NOW)
    payload_hash = live._canonical_payload_hash("sync_leaves", {"sync_action": "sync_all"})
    live._remember_idempotency(
        replay_state, "sync-request-1", action="sync_leaves", actor="tester",
        payload_hash=payload_hash, result={"sync": {"action": "sync_all"}, "merge": {}}, now=NOW,
    )
    monkeypatch.setattr(live, "_read_state", lambda *_args, **_kwargs: (replay_state, 7))
    live.install_live_tour_routes(
        replay_app, engine_instance=RouteEngine, current_identity=lambda: RouteIdentity(),
        require_feature=lambda *_args: None, feature_allowed=lambda *_args: True,
        identity_type=RouteIdentity, google_client=lambda: object(), leave_sheet_id="leave-sheet",
    )
    replay_endpoint = next(
        route.endpoint for route in replay_app.routes
        if getattr(route, "path", "") == "/v2/live-tour/action"
    )

    response = replay_endpoint(body, RouteIdentity())
...
def test_all_representative_mutation_groups_require_idempotency_keys_at_api_boundary():
    assert {
        ...
        "backup", "restore", "merge_current_tour", "sync_leaves",
...
def test_prepared_workbook_rows_are_json_safe_for_durable_sync_marker():
    ...
    sheet.title = "Input"
    sheet.cell(20, 1, "Tên nhân viên")
    sheet.cell(20, 2, "Giờ Booking")
    sheet.cell(20, 3, "Giờ ra")
    sheet.cell(21, 1, "Nguyễn An *")
    sheet.cell(21, 2, datetime(2026, 9, 5, 13, 30))
    sheet.cell(21, 3, clock_time(14, 15))
    output = BytesIO()
    workbook.save(output)

    _, records = live._tour_records_from_xlsm(output.getvalue())

    assert records[0]["Giờ Booking"] == "2026-09-05T13:30:00"
    assert records[0]["Giờ ra"] == "14:15:00"
    assert '"records"' in __import__("json").dumps({"records": records}, ensure_ascii=False)


def test_patch_success_followed_by_verify_timeout_is_commit_uncertain():
    class Response:
        status_code = 200

        @staticmethod
        def json():
            return {"name": "TourVera.xlsm"}
    ...
    prepared = {"target_sha256": "a" * 64, "records": [], "sync_result": {}}
    with pytest.raises(live.LiveTourSyncUncertain) as error:
        live._upload_live_tour_and_verify(
            Session(), b"updated", '"etag"', "file-id", prepared=prepared,
        )

    assert error.value.prepared["target_sha256"] == "a" * 64
    assert "xác minh" in str(error.value)


def test_sync_merge_preserves_active_work_state_and_reason_only_never_changes_status():
    active = employee("e1", "An")
    active.update({"service": "Body 90", "room": "1.1", "status": "Đang thực hiện"})
    state = state_with(active)
    record = {
        "STT": 1, "Tên nhân viên": "An", "Đi làm": "Nghi phep", "Vào ca": "Ca 2",
        "Lịch hẹn": "Nghỉ có phép", "Dịch vụ": "", "Trạng thái": "",
    }

    merged = live._merge_source_records(
        state, [record], NOW, include_assignments=False, sync_action="sync_all",
        field_manifest={"an": {"work_status": True, "appointment": True}},
    )

    assert active["work_status"] == "Đi làm"
    assert active["shift"] == "Ca 1"
    assert active["appointment"] == ""
    assert merged["conflict_count"] == 1

    idle = employee("e2", "Bình")
    state = state_with(idle)
    live._merge_source_records(
        state, [{**record, "Tên nhân viên": "Bình"}], NOW,
        include_assignments=False, sync_action="update_reasons",
    )
    assert idle["work_status"] == "Đi làm"
    assert idle["shift"] == "Ca 1"
    assert idle["appointment"] == "Nghỉ có phép"


def test_all_vba_leave_actions_are_canonical_and_reconcile_live_drift_without_tour_diff():
    canonical_actions = {
        "late_to_working": "Đi làm", "late_to_leave": "Nghỉ phép",
        "early_to_leave": "Nghỉ phép", "early_to_working": "Đi làm",
        "leave_group_to_leave": "Nghỉ phép", "support_to_working": "Đi làm",
    }
    for action, target_status in canonical_actions.items():
        assert live._canonical_sync_action(action) == action
        current_status = "Nghỉ phép" if target_status == "Đi làm" else "Đi làm"
        worker = employee("e1", "An")
        worker["work_status"] = current_status
        state = state_with(worker)
        record = {
            "_source_row": 21, "Tên nhân viên": "An", "Đi làm": target_status,
            "Vào ca": "Ca 2", "Lịch hẹn": "", "Dịch vụ": "", "Trạng thái": "",
        }
        selected, manifest = live._filter_sync_records(
            [record], sync_action=action, source_rows=None, target_date=NOW.date(),
            stats={"affected_rows": [21]}, field_manifest={"an": {"work_status": False}},
        )
        live._merge_source_records(
            state, selected, NOW, include_assignments=False,
            sync_action=action, field_manifest=manifest,
        )
        assert worker["work_status"] == target_status
        assert worker["shift"] == "Ca 1"

    drifted = employee("e1", "An")
    drifted["work_status"] = "Đi làm"
    state = state_with(drifted)
    record = {
        "_source_row": 21, "Tên nhân viên": "An", "Đi làm": "Nghỉ phép",
        "Vào ca": "Ca 2", "Lịch hẹn": "Nghỉ có phép", "Dịch vụ": "", "Trạng thái": "",
    }
    selected, manifest = live._filter_sync_records(
        [record], sync_action="sync_all",
        source_rows=[[NOW.date(), "", "An", "Nghỉ có phép"]], target_date=NOW.date(),
        stats={"affected_rows": [21]}, field_manifest={"an": {"work_status": False}},
    )
    live._merge_source_records(
        state, selected, NOW, include_assignments=False,
        sync_action="sync_all", field_manifest=manifest,
    )
    assert drifted["work_status"] == "Nghỉ phép"
    assert drifted["shift"] == "Ca 1"

    cleanup = employee("e1", "An")
    cleanup["work_status"] = "Nghỉ phép"
    state = state_with(cleanup)
    cleanup_record = {**record, "Đi làm": "Đi làm"}
    selected, manifest = live._filter_sync_records(
        [cleanup_record], sync_action="clear_leave_status", source_rows=None,
        target_date=NOW.date(), stats={"affected_rows": [21]}, field_manifest={},
    )
    live._merge_source_records(
        state, selected, NOW, include_assignments=False,
        sync_action="clear_leave_status", field_manifest=manifest,
    )
    assert cleanup["work_status"] == "Đi làm"


def test_same_sync_key_reconciles_target_hash_and_skips_second_patch(monkeypatch):
    state = state_with(employee("e1", "An"))
    payload = {"sync_action": "sync_all"}
    payload_hash = live._canonical_payload_hash("sync_leaves", payload)
    prepared = {
        "target_sha256": "b" * 64, "original_sha256": "a" * 64,
        "records": [{"STT": 1, "Tên nhân viên": "An", "Đi làm": "Nghi phep", "Vào ca": "Ca 1"}],
        "sync_result": {
            "action": "sync_all", "stats": {},
            "target": {"snapshot_sha256": "b" * 64, "verified": False},
        },
    }
    state["idempotency"]["sync-retry-1"] = {
        "action": "sync_leaves", "actor": "tester", "payload_hash": payload_hash,
        "status": "recovery_required", "patch_attempted": True,
        "prepared": deepcopy(prepared), "result": {},
    }
    state["sync_status"] = {
        "status": "recovery_required", "idempotency_key": "sync-retry-1",
    }
    shared = {"state": state, "revision": 3}

    def read_state(*_args, **_kwargs):
        return deepcopy(shared["state"]), shared["revision"]

    def write_state(_conn, next_state, revision, _actor):
        assert revision == shared["revision"]
        shared["state"] = deepcopy(next_state)
        shared["revision"] += 1
        return shared["revision"]

    monkeypatch.setattr(live, "_read_state", read_state)
    monkeypatch.setattr(live, "_write_state", write_state)
    monkeypatch.setattr(live, "_current_tour_sha256", lambda: ("b" * 64, b"verified"))
    monkeypatch.setattr(
        live, "_run_leave_sync",
        lambda **_kwargs: pytest.fail("same-key recovery must not PATCH Drive again"),
    )
    app = FastAPI()
    live.install_live_tour_routes(
        app, engine_instance=RouteEngine, current_identity=lambda: RouteIdentity(),
    ...
    response = endpoint(
        live.LiveTourAction(action="sync_leaves", idempotency_key="sync-retry-1", payload=payload),
        RouteIdentity(),
    )

    assert response["ok"] is True
    assert response["result"]["sync"]["target"]["recovered"] is True
    assert shared["state"]["idempotency"]["sync-retry-1"]["status"] == "completed"


def test_sync_recovery_write_failure_is_reported_and_drive_io_runs_outside_transaction(monkeypatch):
    class TrackingEngine(RouteEngine):
        depth = 0
    ...
    shared = {"state": state_with(employee("e1", "An")), "revision": 1, "writes": 0}
    ...
    def write_state(_conn, next_state, revision, _actor):
        shared["writes"] += 1
        if shared["writes"] == 3:
            raise RuntimeError("database unavailable while marking recovery")
        shared["state"] = deepcopy(next_state)
        shared["revision"] = revision + 1
        return shared["revision"]

    def uncertain_sync(*, before_upload, **_kwargs):
        assert engine.depth == 0
        prepared = {
            "target_sha256": "c" * 64, "original_sha256": "a" * 64,
            "records": [], "sync_result": {
                "action": "sync_all", "target": {"snapshot_sha256": "c" * 64},
            },
        }
        before_upload(prepared)
        raise live.LiveTourSyncUncertain("verify timeout", prepared=prepared)

    monkeypatch.setattr(live, "_read_state", read_state)
    monkeypatch.setattr(live, "_write_state", write_state)
    monkeypatch.setattr(live, "_run_leave_sync", uncertain_sync)
    app = FastAPI()
    live.install_live_tour_routes(
        app, engine_instance=lambda: engine, current_identity=lambda: RouteIdentity(),
        require_feature=lambda *_args: None, feature_allowed=lambda *_args: True,
        identity_type=RouteIdentity, google_client=lambda: object(), leave_sheet_id="leave-sheet",
    )
    endpoint = next(route.endpoint for route in app.routes if getattr(route, "path", "") == "/v2/live-tour/action")
    ...
    with pytest.raises(HTTPException) as error:
        endpoint(
            live.LiveTourAction(
                    action="sync_leaves", idempotency_key="sync-failure-1",
                    expected_revision=1,
                payload={"sync_action": "sync_all"},
            ),
            RouteIdentity(),
    ...
    assert error.value.status_code == 503
    assert error.value.detail["code"] == "LIVE_TOUR_SYNC_COMMIT_UNCERTAIN"
    assert error.value.detail["marker_persisted"] is False
    assert error.value.detail["ok"] is False
