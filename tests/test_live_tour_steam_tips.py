...
def test_steam_only_needs_service_and_keeps_tours_untouched():
    ...
    result = live._apply_action(state, 'quick_checkout', {'quick_booking': {'service_items': [{'service_id': service['id'], 'quantity': 1}]}}, 'admin', NOW)
...
def test_duplicate_tip_cards_are_preserved_and_summed():
    ...
    result = live._apply_action(state, 'quick_checkout', {'tip_card_ids': ['200k', '200k'], 'quick_booking': {'service_items': [{'service_id': service['id'], 'quantity': 1}]}}, 'admin', NOW)
...
def test_staffed_service_still_requires_employee_and_room():
    ...
    with pytest.raises(HTTPException):
        live._apply_action(state, 'quick_checkout', {'quick_booking': {'service_items': [{'service_id': service['id'], 'quantity': 1}]}}, 'admin', NOW)
...
def test_steam_can_use_existing_customer_combo():
    ...
    invoice = live._apply_action(state, 'quick_checkout', {'customer_id': bought['customer']['id'], 'combo_purchase_id': bought['purchase']['id'], 'payment_method': 'COMBO', 'quick_booking': {'service_items': [{'service_id': service['id'], 'quantity': 1}]}}, 'admin', NOW)['invoice']
